...
public class CommandManager {
    ...
    /**
     * 同步天气信息
     * @param weather  0多云 1晴天 2雪天 3雨天     0 cloudy 1 sunny day 2 snowdays 3 rainy days
     * @param temp  0(0度以上)  1(0度以下)    0 above zero degrees     1 below zero degrees
     */
...
//        data[6] = (byte)0;//占位符
   ...
   /* public void smartWarnInfo(int MessageId, int type, String data) {
    ...
    public void realTimeAndOnceMeasure(int status, int control) {
        ...
        bytes[3] = (byte) 0xFF;
        bytes[4] = (byte) 0x31;
        //心率：0X09(单次) 0X0A(实时)  血氧：0X11(单次) 0X12(实时) 血压：0X21 0X22   // body temp 0X81
        bytes[5] = (byte) status;
    ...
    public void realTimeAndOnceMeasureOff(int status, int control) {
        byte[] bytes = new byte[7];
        bytes[0] = (byte) 0xAB;
        bytes[1] = (byte) 0;
        bytes[2] = (byte) 4;
...
*/
    ...
    public void shakePicture(int control) {
        byte[] bytes = new byte[7];
        bytes[0] = (byte) 0xAB;
        bytes[1] = (byte) 0;
        bytes[2] = (byte) 4;
